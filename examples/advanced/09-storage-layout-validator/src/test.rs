...
fn test_compatible_addition() {
    ...
    // The plan retains `admin` and adds `supply`.
    assert_eq!(report.steps.len(), 2);
    assert_eq!(report.steps.get(0).unwrap().op, MigrationOp::Retain);
    assert_eq!(report.steps.get(1).unwrap().op, MigrationOp::Add);
}
...
fn test_incompatible_type_change() {
    ...
    let report = client.validate(&current, &next);
    assert!(!report.compatible);
    assert_eq!(report.steps.get(0).unwrap().op, MigrationOp::TypeChange);
}
...
fn test_key_collision_detection() {
    ...
    assert_eq!(collisions.get(0).unwrap(), symbol_short!("fee"));

    // A collision in `next` makes the whole upgrade incompatible.
    let good = layout(&env, 1, &[field(symbol_short!("fee"), FieldType::U32)]);
    let report = client.validate(&good, &broken);
    assert!(!report.compatible);
